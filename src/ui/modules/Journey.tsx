...
const Journey: React.FC<JourneyProps> = ({ heading, content, block1, block2, block3, block4, block5, block6, block7, endContent }) => {
  ...
  return (
    <>
      <section ref={sectionRef} className="bg-beige overflow-hidden py-40 md:py-80">
        <div className="fp-container">
          <div className="mx-auto w-full max-w-846">
            {(heading || content) &&(
              <div className="mb-28 md:mb-64">
                {heading && (
                  <h2 className="h1 mb-16 md:mb-40">{heading}</h2>
                )}
                {content && (
                  <p className="fp-text-body">{content}</p>
                )}
              </div>
            )}

            <div className="relative z-1 [&>*:last-child]:mb-0">
              {/* Decor Line Desktop */}
