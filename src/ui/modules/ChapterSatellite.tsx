...
interface ChapterSatelliteProps {
  title: string
  subtitle?: string
  videos: Sanity.Video[]
...
const Video = ({ video }: { video: Sanity.Video }) => {
  return video?.asset?.url && <video autoPlay loop muted playsInline className="absolute -top-1 -left-1 h-[101%] w-[101%] object-cover" src={video.asset.url} />
}

export const ChapterSatellite: React.FC<ChapterSatelliteProps> = ({ title, subtitle, videos, bgColor, ...props }) => {
  ...
  return (
    <section ref={sectionRef} className={`relative overflow-hidden ${bgColor || `bg-cultured`}`} {...moduleProps(props)}>
      <div
        className="fp-container relative z-10 flex min-h-(--chapter-h) flex-col items-center justify-center gap-40 pt-40 pb-60 md:min-h-svh md:gap-20"
        style={{ '--chapter-h': vH } as React.CSSProperties}
      >
        <div className="fp-row">
          <div className="col-span-12 xl:col-span-10 xl:col-start-2">
            <div className="flex flex-col items-center justify-center gap-20 py-16 text-center xl:py-40">
              ...
              <h2 className="h0 md:text-60 whitespace-pre-wrap">{title}</h2>
            </div>
          </div>
        </div>
        {hasVideos && (
          <div className="fp-row">
            <div className="fp-row-container grid grid-cols-24 gap-3 md:gap-6">
              {/* First column - 2 videos */}
              <div className="col-span-6 flex flex-col items-end justify-center gap-3 md:gap-6">
                {videos[0] && (
                ...
                {videos[1] && (
                  <div className="relative aspect-square w-[54%] overflow-hidden">
                    <Video video={videos[1]} />
                  </div>
                )}
              </div>

              {/* Second column - 3 videos */}
              <div className="col-span-11 grid grid-cols-24 gap-3 md:gap-6">
                {videos[2] && (
                  <div className="relative col-span-24 aspect-[396/223] w-full overflow-hidden">
                    <Video video={videos[2]} />
                  </div>
                )}
                {videos[3] && (
                  <div className="relative col-span-9 aspect-square w-full overflow-hidden">
                    <Video video={videos[3]} />
                  </div>
                )}
                {videos[4] && (
                  <div className="relative col-span-15 aspect-square w-full overflow-hidden">
                    <Video video={videos[4]} />
                  </div>
                )}
              </div>

              {/* Third column - 2 videos */}
              <div className="col-span-7 flex flex-col items-start justify-center gap-3 md:gap-6">
                {videos[5] && (
                  <div className="relative aspect-square w-full overflow-hidden">
                    <Video video={videos[5]} />
                  </div>
                )}
                {videos[6] && (
                  <div className="relative aspect-square w-[62%] overflow-hidden">
                    <Video video={videos[6]} />
                  </div>
                )}
              </div>
            </div>
...
export default ChapterSatellite
