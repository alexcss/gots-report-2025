...
export default function DraftModeControls({ globalModules }: { globalModules?: Sanity.GlobalModule[] }) {
    ...
    <details className="frosted-glass fixed right-4 bottom-0 rounded-t bg-amber-200/90 text-xs shadow-xl not-hover:opacity-50 open:opacity-100">
      ...
      <menu className="anim-fade-to-r p-2 pt-0">
        {filteredGlobalModules?.map(({ _id, path }) => {
          ...
          return (
            <li key={_id}>
              <button className="inline-flex items-center gap-1 py-0.5" data-sanity={attr().toString()}>
        ...
        <li>
          <Link className="inline-flex items-center gap-1 py-0.5 hover:underline" href="/api/draft-mode/disable">
